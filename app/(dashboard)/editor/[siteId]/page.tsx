...
export default function EditorPage() {
  ...
  const insertComponentTypes = [
    ...
    { id: 'text-banner', name: 'Text Banner', icon: Type, description: 'Banner with title, description & button' },
  ...
  const handleInsertComponent = (type: string) => {
    ...
    if (type === 'banner-minimal' || type === 'text-banner') {
      const newSection = {
        id: `section-${Date.now()}`,
    ...
    switch (type) {
      case 'text-banner':
        return {
          ...
          description: 'Get started with websites now',
  ...
  <div className="flex-1 flex min-h-0 overflow-hidden">
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {/* Unified Navigation - Each page's sections followed by page name */}
                <nav className="flex gap-4">
                  {/* For each page: show its sections first, then the page name */}
                  {pages
                    .filter(page => page.settings?.showInNavbar !== false)
                    .flatMap((page) => {
                      const navItems = [];
      ...
      {showCardGridModal && (
        <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full mx-4 shadow-2xl border border-gray-200">
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg border-2 border-gray-200">
              ...
              <div className="bg-white p-3 rounded shadow-sm flex items-center justify-center gap-6">
                ...
                <div className="flex gap-4">
                  {/* For each page: show its sections first, then page name */}
                  {pages
                    ...
                    .flatMap(page => {
                      ...
                      items.push(
                        <div
                          key={page._id}
                          className="text-sm text-gray-700 hover:text-blue-600 cursor-pointer transition-colors font-semibold"
                        >
                          {page.pageName}
                        </div>
                      );
